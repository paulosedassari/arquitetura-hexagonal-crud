...
import br.com.cashme.application.dto.PadraoErroDto;
...
public class HandlerException {
    ...
    private ResponseEntity<PadraoErroDto> responseException(Exception e, HttpServletRequest request, HttpStatus codeStatus, String msg) {
        String dataHora = DataUtil.converterParaFormatoEspecificado(LocalDateTime.now(), Constants.FORMATTER_DD_MM_YYYY_HH_MM_SS);
        return ResponseEntity.status(codeStatus).body(new PadraoErroDto(dataHora, msg, e.getMessage(), request.getRequestURI()));
    }

    @ExceptionHandler(CashMeException.class)
    private ResponseEntity<PadraoErroDto> cashMeException(CashMeException e, HttpServletRequest request) {
    ...
    private ResponseEntity<PadraoErroDto> constraintViolationException(RegistroNaoEncontradoException e, HttpServletRequest request) {
    ...
    private ResponseEntity<PadraoErroDto> constraintViolationException(ValidationException e, HttpServletRequest request) {
